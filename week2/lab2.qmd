...
ggplot(storm_data, aes(x = precip_wk_in, y = gage_ht)) +
  geom_point()
...
Precipitation was selected as the predictor variable (x) and gage height as the response (y) because we are assuming that the height of the river is a result of precipitation events in the previous week.
...
beta0 <- storm_sum$coefficients

# Sigma of response only
sd(storm_data$gage_ht)
```
...
sigma hat: 2.3- most of data should lie within 4.6 on either side of line

### Interpret model

**Q3: In plain language, describe how you'd interpret** $\beta_0$ **and** $\beta_1$**. What do they represent?**

Beta 0: estimated gage height if no precipitation

Beta 1: for every inch of rain, the increase in gage height
...
# beta_0_hat = coef(storm_mod[0])
y_hat <- 3.9 + 2.2*x
SST <- sum((storm_data$gage_ht - y_bar)^2)
SSE <- sum((storm_data$gage_ht - y_hat)^2)

# What fraction of variance in gage height have we explained using precipitation
R2 <- (SST - SSE)/SST
...
y is the actual observation of gage height, y hat is the predicted gage height based upon precipitation, and y bar is the mean gage height value over the sample.

**Q5: What does the** $R^2$ **value tell you about the quality of the model (in plain language)?**

The R squared value of 0.55- have removed about half of the variance (how wrong we would be/error based on just the mean gage height)
...
Sigma hat value is 2.3, based upon the model. The sigma value of the response is 3.3, a larger value. The model is capturing a lot of the variability in gage height as the points are generally closer to the predicted value, making predictions more accurate.
...
# or mod_summ$coefficients[2,1] or ["x", "Estimate"]
...
When fitting the model, it does not know the true parameters we supplied (beta 0, beta 1, sigma for example) only the resulting x and y. It uses the data to estimate the hat values of these parameters- using statistical model to reconstruct truth from data alone.

--\> confirming the model works the way you expected.
...
rreas_murres <- read_csv(here::here("week2/rreas.csv"))
...
# Create scatterplot
abundance_diet <- ggplot(rreas_murres, aes(x = comu_rockfish_diet, y = rreas_rockfish_index)) +
  geom_point() +
  labs(x = "Fraction of rockfish in diet",
       y = "Rockfish abundance")
...
I chose fraction of rockfish in diet as the predictor (x) variable and rockfish abundance as the response variable (y). The model would therefore serve to predict how abundant rockfish are based upon how much rockfish common murre are eating.
...
fish_mod <- lm(rreas_rockfish_index ~ comu_rockfish_diet, data = rreas_murres)
...
fish_mod_sum <- summary(fish_mod)

fish_mod_sum$r.squared

```

**Q7: What does the R2 value tell you about the utility of your model for filling in the survey gap?**

The R2 value of 0.698 means we have removed more than half of the variance.

Having fraction of rockfish in diet reduces the error of our predictions by about 70%, making it useful and accurate most of the time for filling in survey gaps as opposed to filling in null values.
...
fish_sigma <- fish_mod_sum$sigma

fish_b0 <- fish_mod_sum$coefficients[1,1]

fish_b1 <- fish_mod_sum$coefficients[2,1]
```

**Q8: In plain language, what do the coefficients and standard deviation of the model represent?**

Beta 0: estimated rockfish abundance if there is no rockfish in the diet of common murre.

Beta 1: When rockfish is the entire diet (fraction = 1), the rockfish abundance increases to 6

Standard deviation: Most of the data should lie within 1.18x2 on either side of the model line.
...
1.19 +- twice the estmated standard deviation, 1.18.
