...
Categorical x-axis vs continuous x-axis, histograms can differ in number of bins.
...
# Returns logicals, so take the mean
...
# Import data
fire_recovery <- read_csv(here::here("week1", "Phenology.csv"))

# Clean data 
fire_recovery_clean <- fire_recovery %>% 
  filter(fire_phase == "post",
         fire_name %in% c("rodeo", "lamesa", "blackhawk", "rattlesnake", "potaz"))

# Bar plot of post fire recovery type
fire_plot <- ggplot(fire_recovery_clean, aes(regrowth, fill = fire_name)) +
  geom_bar() +
  theme_minimal() +
  scale_fill_manual(values = c("#7C6A0A", "#BABD8D", "#FFDAC6", "#FA9500", "#EB6424")) +
  labs(x = "Regrowth type",
       y = "Count",
       title = "Regrowth Observations of Ponderosa Pine Forests Post-Fire",
       subtitle = "1984-2017") 

fire_plot

# Export image
ggsave("fire_plot.png", bg = "white")

```

This graph was created using a USGS data release from the Western Geographic Science Center titled "Phenology pattern data indicating recovery trajectories of ponderosa pine forests after high-severity fires". The data contained sampled areas of ponderosa pine ecosystems across Arizona and New Mexico that experienced high-severity fire, various NDVI measurements, and their vegetative recovery towards a biome type (grassland, shrubland, deciduous, or back to forest) over time.

The bar plot created shows the number of observations in each category of regrowth type over the entire study for five selected high-severity fires. Recovery back to a forest was the most common recorded observation in the post-fire years, with recovery towards a grassland coming second- however, the number of years of observation and number of samples taken varied among fires.

Citation: Walker, J.J., and Soulard, C.E., 2019, Phenology pattern data indicating recovery trajectories of ponderosa pine forests after high-severity fires, U.S. Geological Survey data release, https://doi.org/10.5066/P9Y1Z03F.
