...
fire_recovery_clean <- fire_recovery %>% 
  ...
  filter(fire_phase == "post",
         Fire %in% c("rodeo", "lamesa", "blackhawk", "rattlesnake", "potaz"))

# Bar plot of post fire recovery type
fire_plot <- ggplot(fire_recovery_clean, aes(regrowth, fill = Fire)) +
  geom_bar() +
  theme_minimal() +
...
The bar plot created shows the number of observations in each category of regrowth type over the entire study for five selected high-severity fires. Recovery back to a forest was the most common recorded observation in the post-fire years, with recovery towards a grassland coming second. In this data set, the year of the fire and therefore number of samples taken post-fire varied.
