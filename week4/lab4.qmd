...
Difference in proportions- between sucess/total in garden staple vs popsicle stick method
...
```{r}
#| label: q3
# Group by treatment, create column for proportion (count of successes (coded as 1) over total number of treatments)
...
null_hist <- ggplot() +
  ...
  geom_vline(xintercept = prop_dif, color = "red")
...
# Calculate proportion of where the null distribution value is greater than the obs test stat
p_val <- sum(null_distribution >= prop_dif)/1000
...
Because the p-value was not below 0.05, we cannot reject the hypothesis that restoration method does not influence restoration outcome. Our observed test statistic is not more extreme than the critical threshold. 
...
one_bootstrap <- restoration %>% 
  group_by(treatment) %>% 
  mutate(boot_sample = sample(success,
                      size = length(success),
                      replace = TRUE)) %>% 
  ungroup()
