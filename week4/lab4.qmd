...
Difference in proportions- between success/total in garden staple vs popsicle stick method

### State your null and alternative hypotheses

**Q2: What are your null and alternative hypotheses?**

H0: There is no difference in outcome (restoration success) between the two methods

HA: The popsicle stick method works better than the garden staple method for restoration success.
...
```{r}
#| label: q3
# Group by treatment, create column for proportion (count of successes (coded as 1) over total number of treatments), can also do mean(success)
restoration_summary <- restoration %>% 
  group_by(treatment) %>% 
  summarize(prop_success = sum(success) / n())

prop_dif <- restoration_summary$prop_success[2] - restoration_summary$prop_success[1]
```
...
#| label: q5
#| eval: false
# Make treatment column of newly sampled treatments (shuffle treatment column)
one_permutation <- restoration %>% 
  mutate(treatment = sample(treatment, 
                      size = length(treatment), 
                      replace = FALSE))

# Get proportions from the first permutation using same method
permutation_props <- one_permutation %>% 
  group_by(treatment) %>% 
  summarize(prop_success = sum(success) / n())

# Get difference in props for first permutation
...
```{r}
#| label: q7
# Histogram of null distribution and obs test stat
null_distribution_df <- tibble(null_distribution)
null_hist <- ggplot(null_distribution_df, aes(null_distribution)) +
  geom_histogram(binwidth = 0.05) +
  geom_vline(xintercept = prop_dif, color = "firebrick", linewidth = 1.5)
...
p_val <- mean(null_distribution > prop_dif)
...
Because the p-value was not below 0.05, we fail to reject the hypothesis that restoration method does not influence restoration outcome. Our observed test statistic is not more extreme than the critical threshold.
...
**Q11: What is the appropriate test statistic for this question?** Difference in proportions
...
```{r}
#| label: q14
# Group by trial and treatment, calculate success of each treatment within trials
boot_diff_prop <- bootstrap_samples %>% 
  group_by(trial, treatment) %>% 
  summarize(prop_success = mean(success),
            .groups = "drop_last") %>% # just group by trial now, not treatment
  summarize(diff_prop = prop_success[2] - prop_success[1]) # subtract each garden prop success from popsicle prop success and make new diff_prop column
```
...
# Histogram of null distribution, obs test stat, and CI
ggplot(boot_diff_prop, aes(diff_prop)) +
  geom_histogram() +
  geom_vline(xintercept = prop_dif, 
             color = "firebrick",
             linewidth = 1.5) +
...
**Q18: What did you do to make the null distribution center on zero? Specifically, what code?** Broke the association between treatment and outcome- sampled treatment without replacement.

**Q19: What did you do to make the bootstrap distribution center on the observed test statistic? Specifically, what code?** Retained association between treatment and outcome- grouped by treatment, then sampled with replacement.

**Q20: What would happen to your bootstrap distribution if you sampled without replacement?** Sampling without replacement is just shuffling. Since we sampled within treatments, the bootstrap samples would always match the observed sample. So there would be no variation in the bootstrap distribution of the test stat.
