...
```{r}
#| label: fig-explore
hisp_groups <- drinking_water %>% 
  group_by(pcthisp_bin) %>% 
  summarize(prop_hisp = sum(violation)/n())

ggplot(hisp_groups, aes(x = pcthisp_bin, y = prop_hisp)) +
  geom_point() +
  labs(x = "Percent hispanic bin",
       y = "Proportion of districts with violations")
```

```{r}
pov_groups <- drinking_water %>% 
  group_by(pctpov_bin) %>% 
  summarize(prop_pov = sum(violation)/n())

ggplot(pov_groups, aes(x = pctpov_bin, y = prop_pov)) +
  geom_point() +
  labs(x = "Percent poverty bin",
       y = "Proportion of districts with violations")
```
...
0.01400774 0.01993638

**Based on the sign, what is the relationship between class and SDWA violations?**

as % below poverty increases so do SDWA violations- sign is positive so a positive association
...
# A grid of predictors to generate predictors for
pred_grid <- expand_grid(pctpov = 0:100)

# Generate our predictions- response undoes logit
...
class_model_int <- glm(violation ~ pctpov*pcthisp, # formula: response ~ predictor)
                   data = drinking_water, # data 
                   family = binomial(link = "logit")) # family and link function
```

```{r}
# 95% CI for % Hispanic coefficient
confint(class_model_int)["pcthisp",]
summary(class_model_int)
```

-0.009529405 -0.001782473

The interaction does not seem to be significantly different than 0. (3.246e-04)

```{r}
# A grid of predictors to generate predictors for
pred_grid_2 <- expand_grid(pctpov = 0:100,
                           pcthisp = 0:100)

# Generate our predictions- response undoes logit
class_model_pred <- pred_grid_2 %>% 
  mutate(p = predict(object = class_model_int,
                     newdata = pred_grid_2,
                     type = "response"))
```

```{r}
class_model_int_se <- predict(object = class_model_int,
                          newdata = pred_grid_2,
                          type = "link", # stay in link space!
                          se.fit = TRUE) # get the se of the fit

# Go from link space to response space
linkinv <- family(class_model_int)$linkinv

class_model_predint <- pred_grid_2 %>% 
  mutate(
    # get logit(p)
    logit_p = class_model_int_se$fit,
    # 95% CI in link space- keeps in realistic range of values!! can do math here
    logit_p_se = class_model_int_se$se.fit,
    logit_p_lwr = qnorm(0.025, mean = logit_p, sd = logit_p_se),
    logit_p_upr = qnorm(0.975, mean = logit_p, sd = logit_p_se),
    # undo the link function using linkinv- can interpret here
    p = linkinv(logit_p),
...
# filter predictions to a few levels
class_model_predint_filtered <- class_model_predint %>%
  filter(pctpov %in% c(0, 25, 50, 75)) %>%
  mutate(pctpov = factor(pctpov))

# plot filtered predictions, colored by pctpov
ggplot(class_model_predint_filtered, aes(x = pcthisp, y = p, fill = pctpov))+
  geom_ribbon(aes(ymin = p_lwr, ymax = p_upr), alpha = 0.2) +
  geom_line()
```

```{r}
summary(class_model_pred_int)
```
...
SDWA violations depend on both the percent Hispanic and percent poverty in the district. Quantitatively based upon the prediction figure, the relationship between the probability of an SDWA violation and percent Hispanic in the district depends upon the percent poverty. The confidence interval for when percent poverty is at the highest level (75) is widest and most positive while the CI for when percent poverty is at the lowest level (0) it the narrowest and most negative. Based upon the model summary, the CI for the coefficient of percent Hispanic was very close to 0. The interaction term between percent poverty and percent Hispanic was also not significantly different than 0. These statistical outputs could be misinterpreted because the overall effect of percent Hispanic on violations matters but varies in sign based upon the percent poverty, which might cause the effects to cancel out when one variable is not considered even though there is a relationship as shown in the prediction figure.
